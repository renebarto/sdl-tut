...
public:
    ...
    bool Init()
    {
        ...
        if (!LoadVertexShader("VertexShader.txt"))
            return false;

        if (!LoadFragmentShader("FragmentShader.txt"))
            return false;

        // All shaders have been created, now we must put them together into one large object
        return LinkShaders();
    }
